...
paragraph/list/table AST with emphasis and semantic references, complete
provenance, explicit canonical/source/fallback labels and aliases, and
...
blob/hash. Canonical serialization produces a `dwb-domain-catalog` envelope,
content-addressed core/glossary/faction indexes, and entity buckets selected by
successive SHA-256 bytes. Every chunk is limited to 512 KiB and paired with
lookup indexes plus a schema-semver/content-version contract.
...
descriptor integrity, content version, strict payload schemas, hash-bucket
membership, indexes, persisted view coverage, lookup coverage, and cross-record
reference closure before exposing a catalogue.
